...
const uint32_t TABLE_MAX_ROWS = ROWS_PER_PAGE * 100;
...
void serialize_row(Row* source, void* destination) {
    __builtin___memcpy_chk (destination + ID_OFFSET, &(source->id), ID_SIZE, __builtin_object_size (destination + ID_OFFSET, 0));
    __builtin___memcpy_chk (destination + USERNAME_OFFSET, &(source->username), USERNAME_SIZE, __builtin_object_size (destination + USERNAME_OFFSET, 0));
    __builtin___memcpy_chk (destination + EMAIL_OFFSET, &(source->email), EMAIL_SIZE, __builtin_object_size (destination + EMAIL_OFFSET, 0));
}

void deserialize_row(void* source, Row* destination) {
    __builtin___memcpy_chk (&(destination->id), source + ID_OFFSET, ID_SIZE, __builtin_object_size (&(destination->id), 0));
    __builtin___memcpy_chk (&(destination->username), source + USERNAME_OFFSET, USERNAME_SIZE, __builtin_object_size (&(destination->username), 0));
    __builtin___memcpy_chk (&(destination->email), source + EMAIL_OFFSET, EMAIL_SIZE, __builtin_object_size (&(destination->email), 0));
}
# 128 "src/lib.h"
...
void read_input(InputBuffer* input_buffer) {
    ssize_t bytes_read = getline(&(input_buffer->buffer), &(input_buffer->buffer_length), __stdinp);
